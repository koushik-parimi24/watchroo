...
function MovieDetails() {
    ...
    const fetchDetails = async () => {
      try {
        ...
        if (isTV && data?.seasons?.length) {
          const firstRealSeason = data.seasons.find(s => s.season_number > 0) ?? { season_number: 1 };
  ...
  const streamingServers = {
    server1: {
      ...
      url: (id, type, s, e) =>
        type === 'tv'
          ? `https://www.2embed.cc/embedtv/${id}/${s}/${e}`
          : `https://www.2embed.cc/embed/${type}/${id}`,
      downloadUrl: (id, type) =>
        type === 'movie' ? `https://www.2embed.cc/get/${id}.mp4` : null,
      quality: 'HD',
      ads: 'Minimal',
    },
    server2: {
      ...
      downloadUrl: (id, type) =>
        type === 'movie' ? `https://vidsrc.me/file/${id}.mp4` : null,
      quality: 'HD+',
      ads: 'Few',
    },
    server3: {
  ...
  const getCurrentStreamUrl = () => {
    const srv = streamingServers[selectedServer];
    return srv ? srv.url(id, type, selectedSeason, selectedEpisode) : '';
  };

  const getCurrentDownloadUrl = () => {
    const srv = streamingServers[selectedServer];
    if (!srv?.downloadUrl) return null;
    try {
      return srv.downloadUrl(id, type, selectedSeason, selectedEpisode);
    } catch {
      return null;
    }
  };
  ...
  const trailer =
    item.videos?.results?.find(v => v.type === 'Trailer' && v.site === 'YouTube') ||
